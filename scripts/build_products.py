"""
build_products.py  - 用 hardcoded 資料快速生成完整 products.json（不需要網路）
"""
import json
from pathlib import Path
from datetime import datetime, timezone

OUT = Path(__file__).parent.parent / "data" / "products.json"

ZH_NAMES = {
    "BX-01":"蒼龍斬刃","BX-02":"地獄鐮刀","BX-03":"魔法弓矢",
    "BX-04":"騎士之盾 B","BX-05":"魔導幻箭","BX-06":"騎士重盾",
    "BX-07":"新手入門組","BX-08":"3對3甲板組","BX-09":"戰鬥通行證",
    "BX-10":"極限競技場","BX-11":"X發射器握把","BX-12":"隨機強化組 Vol.2",
    "BX-13":"騎士長矛",          # KNIGHTLANCE 單獨商品（phstudy確認）
    "BX-14":"隨機強化組 Vol.3",  # 六入隨機包（含舊版陀螺，非暗鬼巨錘）
    "BX-15":"雄獅獵爪",          # LEONCLAW（論壇BX-24-05確認）
    "BX-16":"毒蛇鋒尾",          # VIPERTAIL 三色隨機包（非霸王狂鯊）
    "BX-17":"雙入強化組",        # DRANSWORD+WIZARDARROW 雙入包（非鳳凰飛翼B）
    "BX-18":"鐵甲傑克",
    "BX-19":"犀牛猛角",          # RHINOHORN（非魔法弓矢2）
    "BX-20":"三入強化組",        # 三入隨機包（非狂鯊銳齒）
    "BX-21":"隨機強化組 Vol.4",
    "BX-22":"蒼龍斬刃 改",       # DRANSWORD 特別版單獨商品（非隨機包）
    "BX-23":"鳳凰飛翼","BX-24":"六入強化組",  # BX-24 六入隨機包（非龍王閃擊）
    "BX-25":"陀螺收納包","BX-26":"獨角刺穿",  # UNICORNSTING（非蒼龍斬刃2）
    "BX-27":"獅身甲隨機組",      # SPHINXCOWL 三色隨機包（非地獄鐵鏈）
    "BX-28":"魔像岩錘","BX-29":"騎士聖盾","BX-30":"X發射器改造型握把",
    "BX-31":"天馬爆擊 改","BX-32":"超寬競技場",
    "BX-33":"白虎迅雷",          # WEISSTIGER（非地獄鐮刀2）
    "BX-34":"蒼穹龍騎士",        # COBALTDRAGOON（論壇BX-48-01確認）
    "BX-35":"六入強化組 Vol.2",  # 六入隨機包（非鳳凰飛翼2）
    "BX-36":"巨鯨怒濤",          # WHALEWAVE（論壇CX-05-06確認）
    "BX-37":"熊掌爪擊",          # BEARSCRATCH（非魔法弓矢3）
    "BX-38":"赫然天鳳","BX-39":"龍神決戰組 Vol.1",
    "BX-40":"發條發射器L","BX-41":"橡膠改造握把 黑版","BX-42":"橡膠改造握把 藍版",
    "BX-43":"陀螺收納盒","BX-44":"三角強襲","BX-45":"武士魂斬",
    "BX-46":"對戰入門組","BX-47":"發射器L 紅版","BX-48":"隨機強化組 Vol.9",
    "BX-49":"蒼龍突襲","BX-50":"隨機強化組 Vol.11","BX-51":"發射器 黑綠版",
    "UX-01":"蒼龍爆刃","UX-02":"惡魔戰錘","UX-03":"魔導神杖",
    "UX-04":"衝擊龍神 改","UX-05":"蠍子神矛 改","UX-06":"雄獅巔峰",
    "UX-07":"霜輝銀狼 2","UX-08":"霜灰銀狼","UX-09":"武士星劍 豪華組",
    "UX-10":"騎士圓甲","UX-11":"衝擊龍神","UX-12":"天馬爆擊 豪華組",
    "UX-13":"魔像奇岩","UX-14":"天蠍長矛","UX-15":"鮫鯊狂麟",
    "UX-16":"時鐘幻象","UX-17":"殞星龍騎士","UX-18":"隨機強化組 Vol.8",
    "UX-19":"子彈獅鷲","UX-20":"榮耀女武神",
    "CX-01":"蒼龍勇氣","CX-02":"魔導至尊","CX-03":"冥界黑暗",
    "CX-04":"對戰入門組C","CX-05":"隨機強化組 Vol.6","CX-06":"狐狸精選組",
    "CX-07":"天馬爆擊","CX-08":"隨機強化組 Vol.7","CX-09":"天馬爆擊 改",
    "CX-10":"銀狼狩獵","CX-11":"帝王威能組","CX-12":"鳳凰閃焰",
    "CX-13":"龍王閃擊","CX-14":"騎士堡壘","CX-15":"邪神狂怒",
    "CX-16":"異色龍王決鬥盤組","CX-17":"隨機強化組 Vol.10","CX-18":"腕龍鞭打",
}

TYPE_MAP = {
    "BX-01":"attack","BX-02":"defense","BX-03":"stamina","BX-04":"defense",
    "BX-13":"defense",   # 騎士長矛 KNIGHTLANCE
    "BX-15":"balance",   # 雄獅獵爪 LEONCLAW
    "BX-19":"balance",   # 犀牛猛角 RHINOHORN
    "BX-22":"attack",    # 蒼龍斬刃 改 DRANSWORD special
    "BX-23":"attack","BX-26":"attack",  # 獨角刺穿 UNICORNSTING
    "BX-28":"defense","BX-29":"defense","BX-33":"defense","BX-34":"attack",
    "BX-36":"balance",   # 巨鯨怒濤 WHALEWAVE
    "BX-37":"attack",    # 熊掌爪擊 BEARSCRATCH
    "BX-44":"defense","BX-45":"attack",
    "BX-49":"attack",
    "UX-01":"attack","UX-02":"attack","UX-03":"stamina","UX-06":"balance",
    "UX-07":"balance","UX-08":"balance","UX-10":"defense","UX-11":"stamina",
    "UX-13":"defense","UX-14":"balance","UX-15":"attack","UX-16":"stamina",
    "UX-17":"attack","UX-19":"attack","UX-20":"attack",
    "CX-01":"attack","CX-02":"stamina","CX-03":"defense","CX-07":"attack",
    "CX-09":"attack","CX-10":"balance","CX-11":"defense","CX-12":"attack",
    "CX-13":"attack","CX-14":"defense","CX-15":"attack","CX-16":"balance",
    "CX-18":"balance",
}

TIER_MAP = {
    # S tier = 論壇 TIER0（必買）
    "UX-15":("S",99.0),"UX-03":("S",97.0),"BX-23":("S",95.0),
    "UX-17":("S",93.0),"UX-11":("S",91.0),"UX-19":("S",89.0),
    # A tier = 論壇 TIER1（優先考慮）
    "BX-49":("A",85.0),"CX-18":("A",83.0),"UX-08":("A",81.0),
    "UX-10":("A",79.0),"CX-11":("A",77.0),"UX-02":("A",75.0),
    "CX-07":("A",73.0),"UX-01":("A",71.0),"UX-16":("A",69.0),
    "CX-13":("A",67.0),"BX-36":("A",65.0),"CX-12":("A",63.0),
    # B tier = 論壇 TIER2（有閒錢考慮）
    "UX-06":("B",58.0),"CX-02":("B",55.0),"CX-14":("B",53.0),
    "CX-15":("B",51.0),"CX-09":("B",49.0),"CX-10":("B",47.0),
    "UX-13":("B",45.0),"UX-14":("B",43.0),"BX-24":("B",41.0),
    "BX-16":("B",39.0),"BX-44":("B",37.0),"BX-45":("B",35.0),
    "UX-07":("B",33.0),"BX-35":("B",31.0),
    # C tier = 舊款/入門
    "BX-01":("C",28.0),"BX-02":("C",26.0),"BX-06":("C",24.0),
    "BX-14":("C",22.0),"CX-03":("C",20.0),"BX-20":("C",18.0),
}

RELEASE_DATES = {
    "BX-01":"2023-07-15","BX-02":"2023-07-15","BX-03":"2023-07-15",
    "BX-04":"2023-07-15","BX-05":"2023-07-15","BX-06":"2023-07-15",
    "BX-07":"2023-07-15","BX-08":"2023-07-15","BX-09":"2023-07-15",
    "BX-10":"2023-07-15","BX-11":"2023-10-07","BX-12":"2023-10-07",
    "BX-13":"2024-01-06","BX-14":"2024-01-06","BX-15":"2024-01-06",
    "BX-16":"2024-01-06","BX-17":"2024-01-06","BX-18":"2024-04-06",
    "BX-19":"2024-04-06","BX-20":"2024-04-06","BX-21":"2024-04-06",
    "BX-22":"2024-07-06","BX-23":"2024-01-06","BX-24":"2024-07-06",
    "BX-25":"2024-07-06","BX-26":"2024-07-06","BX-27":"2024-10-05",
    "BX-28":"2024-10-05","BX-29":"2024-10-05","BX-30":"2024-04-06",
    "BX-31":"2024-10-05","BX-32":"2024-04-06","BX-33":"2025-01-11",
    "BX-34":"2025-01-11","BX-35":"2025-01-11","BX-36":"2025-04-05",
    "BX-37":"2025-04-05","BX-38":"2025-04-05","BX-39":"2025-07-05",
    "BX-40":"2025-07-05","BX-41":"2025-07-05","BX-42":"2025-10-04",
    "BX-43":"2025-10-04","BX-44":"2025-10-04","BX-45":"2026-01-11",
    "BX-46":"2026-01-11","BX-47":"2026-04-05","BX-48":"2026-04-05",
    "BX-49":"2026-04-05","BX-50":"2026-07-19","BX-51":"2026-07-19",
    "UX-01":"2024-07-06","UX-02":"2024-07-06","UX-03":"2024-10-05",
    "UX-04":"2024-10-05","UX-05":"2024-10-05","UX-06":"2024-07-06",
    "UX-07":"2024-10-05","UX-08":"2024-10-05","UX-09":"2025-01-11",
    "UX-10":"2024-10-05","UX-11":"2024-12-07","UX-12":"2025-01-11",
    "UX-13":"2025-04-05","UX-14":"2025-07-05","UX-15":"2025-07-05",
    "UX-16":"2025-10-04","UX-17":"2025-10-04","UX-18":"2026-01-11",
    "UX-19":"2026-04-05","UX-20":"2026-07-19",
    "CX-01":"2025-04-05","CX-02":"2025-04-05","CX-03":"2025-04-05",
    "CX-04":"2025-04-05","CX-05":"2025-04-05","CX-06":"2025-07-05",
    "CX-07":"2025-07-05","CX-08":"2025-07-05","CX-09":"2025-04-05",
    "CX-10":"2025-10-04","CX-11":"2025-10-04","CX-12":"2025-10-04",
    "CX-13":"2026-01-11","CX-14":"2026-01-11","CX-15":"2026-04-05",
    "CX-16":"2026-04-05","CX-17":"2026-04-05","CX-18":"2026-07-19",
}

# 台灣官方建議售價（NT$），主要可玩陀螺本體
MSRP_TWD = {
    "BX-01":499,  "BX-02":499,  "BX-03":499,  "BX-04":499,
    "BX-05":499,  "BX-06":499,
    "BX-13":499,  # 騎士長矛
    "BX-14":499,  "BX-15":499,  "BX-16":499,
    "BX-19":499,  "BX-20":499,  "BX-22":499,  "BX-23":699,  "BX-24":699,
    "BX-26":599,  "BX-28":599,  "BX-29":599,
    "BX-33":699,  "BX-34":699,  "BX-35":699,  "BX-36":699,
    "BX-37":699,  "BX-38":699,  "BX-44":899,  "BX-45":899,
    "BX-49":999,
    "UX-01":699,  "UX-02":699,  "UX-03":699,  "UX-04":699,
    "UX-05":699,  "UX-06":699,  "UX-07":699,  "UX-08":699,
    "UX-10":699,  "UX-11":799,  "UX-13":799,  "UX-14":799,
    "UX-15":1299, "UX-16":999,
    "UX-17":899,  "UX-19":999,  "UX-20":999,
    "CX-01":699,  "CX-02":699,  "CX-03":699,  "CX-07":699,
    "CX-09":799,  "CX-10":799,  "CX-11":999,  "CX-12":899,
    "CX-13":999,  "CX-14":999,  "CX-15":999,  "CX-16":1099,
    "CX-18":1099,
}

ACCESSORY = {
    "BX-07","BX-08","BX-09","BX-10","BX-11","BX-12",
    # BX-13 = 騎士長矛（單獨商品，移出隨機包分類）
    "BX-17",  # 雙入強化組（非可排名商品）
    "BX-21","BX-25","BX-27","BX-30","BX-31","BX-32","BX-39",
    # BX-22 = 蒼龍斬刃 改（單獨商品，移出隨機包分類）
    "BX-40","BX-41","BX-42","BX-43","BX-46","BX-47","BX-48","BX-50","BX-51",
    "UX-09","UX-12","UX-18",
    "CX-04","CX-05","CX-06","CX-08","CX-17",
}

# 預購資訊（手動維護）
# stores 欄位：name=通路名稱, url=預購連結, method=預購方式, note=備注
PREORDER_DATA = {
    "UX-20": {
        "available": True,
        "startDate": None,
        "endDate": None,
        "estimatedShipDate": "2026-07-19",
        "stores": [
            {
                "name": "FUNBOX",
                "url": "https://shop.funbox.com.tw/categories/takaratomy/beyblade/XIKBXB",
                "method": "戰鬥陀螺X 強化組",
                "note": "上架後將更新直連預購頁",
            },
        ],
        "notes": "台灣官方代理 FUNBOX，商品頁上架後連結將更新",
    },
    "CX-18": {
        "available": True,
        "startDate": None,
        "endDate": None,
        "estimatedShipDate": "2026-07-19",
        "stores": [
            {
                "name": "FUNBOX",
                "url": "https://shop.funbox.com.tw/categories/takaratomy/beyblade/XIKBXB",
                "method": "戰鬥陀螺X 強化組",
                "note": "上架後將更新直連預購頁",
            },
        ],
        "notes": "台灣官方代理 FUNBOX，商品頁上架後連結將更新",
    },
    # 範例：未來新商品預購資訊格式
    # "BX-52": {
    #     "available": True,
    #     "startDate": "2026-09-01",
    #     "endDate": "2026-09-30",
    #     "estimatedShipDate": "2026-10-10",
    #     "stores": [
    #         {"name": "FUNBOX", "url": "https://...", "method": "線上預購", "note": "限量 500 個"},
    #         {"name": "MOMO", "url": "https://...", "method": "線上預購"},
    #         {"name": "全家便利商店", "method": "到店預購", "note": "需至門市填預購單"},
    #     ],
    #     "notes": "首批限量發售，售完不補",
    # },
}

IMG = "https://beyblade.takaratomy.co.jp/beyblade-x/lineup/_image/{}_list.png"
TODAY = datetime.now(timezone.utc).strftime("%Y-%m-%d")
NOW_ISO = datetime.now(timezone.utc).isoformat()

GM_HISTORY_FILE = Path(__file__).parent.parent / "data" / "gray-market-history.json"

def load_gm_history() -> dict:
    if GM_HISTORY_FILE.exists():
        try:
            return json.loads(GM_HISTORY_FILE.read_text(encoding="utf-8"))
        except Exception:
            pass
    return {"prices": {}}

def build_gray_market(code: str, zh_name: str, gm_history: dict) -> dict | None:
    """Gray market 搜尋 URL + 最新價格（含近 30 天歷史）"""
    if code in ACCESSORY:
        return None
    tier = TIER_MAP.get(code)
    if not tier:
        return None  # 只有有 tier 的商品才顯示水貨區

    from urllib.parse import quote
    kw_tw = f"戰鬥陀螺X {code}"
    kw_jp = f"ベイブレードX {code}"
    history = gm_history.get("prices", {}).get(code, [])
    latest = history[-1] if history else {}
    return {
        "shopeeUrl": f"https://shopee.tw/search?keyword={quote(kw_tw)}",
        "rutenUrl": f"https://www.ruten.com.tw/find/?q={quote(kw_tw)}",
        "yahooJpUrl": f"https://auctions.yahoo.co.jp/search/search?p={quote(kw_jp)}",
        "shopeePrice": latest.get("shopeeMin"),
        "rutenPrice": latest.get("rutenMin"),
        "priceHistory": history[-30:],  # 最近 30 筆
        "lastUpdated": latest.get("date"),
    }

ALL_CODES = (
    [f"BX-{i:02d}" for i in range(1, 52)] +
    [f"UX-{i:02d}" for i in range(1, 21)] +
    [f"CX-{i:02d}" for i in range(1, 19)]
)

gm_history = load_gm_history()

products = []
for code in ALL_CODES:
    key = code.replace("-", "")
    rd = RELEASE_DATES.get(code, "")
    released = (rd <= TODAY) if rd else True
    tier, score = TIER_MAP.get(code, (None, None))
    zh_name = ZH_NAMES.get(code, code)
    products.append({
        "id": code,
        "code": code,
        "nameZh": zh_name,
        "nameJa": "",
        "imageUrl": IMG.format(key),
        "type": TYPE_MAP.get(code) if code not in ACCESSORY else None,
        "series": code.split("-")[0],
        "isAccessory": code in ACCESSORY,
        "tier": tier,
        "tierScore": score,
        "releaseDate": rd,
        "releasedTW": released,
        "priceJPY": None,
        "priceTWD": MSRP_TWD.get(code),
        "marketPriceTWD": None,
        "availability": {
            "momo": None, "pchome": None, "funbox": None,
            "eslite": None, "yahoo": None,
        },
        "preorder": PREORDER_DATA.get(code),
        "grayMarket": build_gray_market(code, zh_name, gm_history),
        "lastUpdated": NOW_ISO,
    })

OUT.write_text(json.dumps(products, ensure_ascii=False, indent=2), encoding="utf-8")
print(f"Done: {len(products)} products written to {OUT}")
